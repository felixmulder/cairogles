			...
			cairo_clip_t *clip)
{
    ...
    cairo_bool_t has_component_alpha;
    cairo_gl_context_t *ctx;
    int old_glyph_mask_height = 0, old_glyph_mask_width = 0;
	...
	/* Prevent situations where we seesaw between a wide and a tall glyph mask. */
	old_glyph_mask_height = ctx->glyph_mask->height;
	old_glyph_mask_width = ctx->glyph_mask->width;
	...
	ctx->glyph_mask = (cairo_gl_surface_t *)
	     cairo_gl_surface_create (dst->base.device,
				      ...
				      MAX (info->extents.width, old_glyph_mask_width),
				      MAX (info->extents.height, old_glyph_mask_height));
